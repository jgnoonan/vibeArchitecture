...
Most privacy laws are triggered by **who your users are**, not by how big or serious your app is. If you have EU/UK users, GDPR applies. If you have US users, California (CCPA/CPRA) and roughly twenty other states now have comprehensive privacy laws with broadly similar rights — thresholds vary, and several have none for sensitive data. A hobby-scale Public app with real signups almost always falls under at least one of these. You don't need to be "Regulated" to owe users these basics.
...
- **Rights around automated decisions:** If software (including an LLM) makes or materially shapes a decision with legal or similarly significant effects on a person — credit, hiring, housing, pricing, account termination — GDPR Art. 22 and the CCPA ADMT regulations (phasing in through 2027) give the person a right to notice, to opt out or contest, to a human review, and to a meaningful explanation of the logic. Architecturally: keep a human-review path, log the inputs and the decision, and never let a model be the sole decider for these.
...
- Honor the **Global Privacy Control** signal (`Sec-GPC: 1` header / `navigator.globalPrivacyControl`). California, Colorado, Connecticut, and other states treat it as a legally binding opt-out of sale/sharing and targeted advertising. Detect it server-side and suppress the relevant trackers and data sharing for that visitor.
...
- If you market "private" or "end-to-end encrypted," the metadata plane (invitation records, push payloads, server logs, analytics events, who-knows-whom columns) must hold up against an honest-but-curious, compromised, or subpoenaed operator, and legacy cleartext paths are deleted, not deprecated. See `guides/security/threat-modeling.md` (The Operator in the Mirror).
...
- Tell users when they are talking to a bot and when content is AI-generated (Public tier and above, all users — not only in the EU). Label AI chat as AI, label synthetic images/audio/video, and don't present model output as a human's words. EU users have a statutory right to this (`rules/compliance.md`, EU AI Act Art. 50); several US states now require it for specific contexts (`rules/compliance.md`, US AI and State Laws).
- If user content may be used to train or fine-tune models — yours or a provider's — say so in the notice and offer an opt-out. Default to providers that don't train on your API traffic (`rules/multi-agent.md`).
...
- Pending: the EU's Digital Omnibus package proposes GDPR amendments (narrower definition of personal data for pseudonymized sets, an AI-training legitimate-interest basis, cookie-consent changes). Proposals only as of August 2026 — track them; don't build to them yet.
