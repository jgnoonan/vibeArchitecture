...
- At Business tier, express reliability as an SLO with an error budget (99.9% success over 30 days = 43 minutes of allowed failure) and alert on **burn rate** — how fast the budget is being consumed — rather than on raw thresholds alone. A fast burn (14× budget rate over 1 hour) pages; a slow burn (6× over 6 hours or 1× over 3 days) opens a ticket. See `guides/observability/monitoring.md`.
...
- Sample traces at volume: head-sample (e.g. 10%, decided at request start) for simplicity, tail-sample (decided after completion, keeping every error and slow trace) when you run a collector; always keep 100% of errors. One OTel SDK/collector carries traces, metrics, and logs. See `guides/observability/monitoring.md` (Trace Sampling).
...
- Use severity levels:
  - **Page** (wake someone up): The service is down, error rate above 5% for 5 minutes, data is being lost, users are impacted right now.
  - **Ticket** (fix within the next business day): Error rate elevated but under 5%, performance degraded, a non-critical component failing, a disk 80% full.
