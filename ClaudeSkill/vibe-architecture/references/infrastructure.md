...
- Define all infrastructure in code (Terraform or OpenTofu, Pulumi, CloudFormation, CDK, or your platform's config files). If it's not in code, it doesn't reliably exist — it can't be reviewed, versioned, or reproduced. "From day one" scales by tier: on a PaaS, the platform config file *is* your IaC; adopt Terraform/OpenTofu when you're composing cloud primitives (VPCs, databases, buckets, IAM) yourself.
...
- Scan infrastructure code in CI (Checkov or Trivy's config scanner — tfsec has been merged into Trivy) and block on high-severity findings. Misconfigured IaC is how public storage buckets and wide-open security groups happen — the scanner catches them before they deploy.
...
- Pin third-party CI actions to a full commit SHA with the version as a comment (`uses: actions/checkout@<40-char sha> # v4.2.2`), not a mutable tag. Tags can be moved to malicious commits; a SHA can't. Let Dependabot/Renovate bump the SHA.
...
- Buckets are private by default (public only for assets that are public by design); clients read and write through short-lived presigned URLs (minutes, not days), uploads go directly to the bucket with the signed request constraining content type, size, and key, and you authorize before you sign. See `guides/infrastructure/cloud-fundamentals.md` and `rules/security.md` (File Uploads).

## Domains and DNS

- Domain and DNS: auto-renew on a payment method that won't expire, registrar lock plus 2FA, renewal date on a shared calendar; DNS at a provider with an API and track record (ideally separate from the registrar); DNSSEC where both support it; lower record TTLs to 60–300 seconds a day before any host/CDN/DNS migration. See `guides/operations/day2-operations.md` (Domain, DNS, and Certificates).
