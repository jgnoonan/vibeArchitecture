...
Start with: "Before we start building, I'd like to ask a few quick questions about your project. This helps me apply the right level of protection. Takes about 5 minutes."

**Q0: "What's your background with building software?"** — new to this (AI does most of the coding) → `beginner`; built a few things (can read and change code) → `intermediate`; experienced developer → `experienced`. Record as `experience_level`. This changes communication style only, not which rules apply.
...
- Children's data → upgrade to **Regulated** (COPPA / GDPR-K, age verification)
...
**Q4: "Imagine it goes down completely. What's the impact?"** (Skip for Personal tier)
- No big deal / Annoying → no change
- Serious → note it; consider **Business** if a business depends on it
- Critical (money lost, legal issues, people harmed) → upgrade to at least **Business**; combined with sensitive data → **Regulated**
Record the answer as `downtime_impact` in the profile.

**Q4b: "How will people use it — web browser, mobile app, both, or something else (desktop, API, CLI)?"**
Record as `platform`: web / mobile-native / both / other. A native mobile app (iOS, Android, React Native, Flutter) loads `mobile.md`.
...
Then check for upgrades (tier can only go UP):
  ...
  Q3 has children's data → Regulated
...
- **Downtime impact:** [no big deal / annoying / serious / critical]
...
- Load `compliance.md` at Public tier and above when EU users interact with an AI feature — only the EU AI Act section applies below Regulated tier

Read the relevant rule files from `references/` and follow them for every piece of code you write.

**Before writing any code, confirm what's active** in two or three lines — tier and reason, privacy overlay on/off, exact rule files loaded — and re-confirm when scope changes. This is the same self-check described in `ARCHITECT.md` ("Confirm what's active before writing code") in the full framework; if you can't name the loaded rules, you haven't loaded them.
...
In Claude Code, `PROJECT_PROFILE.md` is a real file in the project root and persists on its own. In Claude.ai, files in the conversation do not carry over between chats: to keep the profile, either use a Claude Project (upload the profile as project knowledge) or keep the file in the user's repository and have them paste or attach it at the start of each session.
...
Install locations: Claude.ai — upload the ZIP under Settings > Capabilities. Claude Code — copy this folder to `.claude/skills/vibe-architecture/` (project) or `~/.claude/skills/vibe-architecture/` (personal).
