...
- **Functions are stateless and spin up per request.** There's no "the server" that stays running. Each invocation may start on a fresh instance.
...
- **No in-process cache you can trust.** A cache in memory is per-instance and vanishes on freeze/redeploy. For shared caching, use an external store (Redis/Upstash, the platform's KV/edge cache, or a CDN).
...
- **A managed pooler in transaction mode** — Supabase's connection pooler, or **PgBouncer** in transaction mode. "Transaction mode" hands a connection back to the pool after each transaction instead of holding it for a whole session, which is exactly what short-lived functions need.
...
- **A shared store** — Redis, or **Upstash** (a serverless-friendly Redis with an HTTP API and ready-made rate-limit libraries). Every invocation increments the same counter.
...
One gotcha specific to **edge runtimes** (Cloudflare Workers, Vercel Edge Functions): they are **not full Node.js**. Some Node built-ins, native modules, filesystem access, and long-lived TCP sockets are unavailable. Libraries that assume a raw database socket often won't run at the edge — which is another reason HTTP-based data APIs pair naturally with edge deployments. Check that your dependencies support the edge runtime before shipping, or keep database-heavy work in standard serverless functions and reserve the edge for lightweight, latency-sensitive logic.
...
| In-process connection pool (5–20 connections)         | External pooler (PgBouncer transaction mode, Neon/Supabase pooled URL, RDS Proxy)   |
| In-memory cache / global variables                    | External store (Redis/Upstash, platform KV, CDN) — per-instance memory is unreliable |
| In-memory rate-limit counter                          | Shared store (Upstash/Redis), platform rate-limit primitive, or WAF/CDN             |
...
| Session state held in server memory                   | External session store (Redis, database, signed cookies / JWT)                      |
...
- [ ] If deploying to the **edge**, verified that dependencies work without full Node APIs and don't need raw sockets.
