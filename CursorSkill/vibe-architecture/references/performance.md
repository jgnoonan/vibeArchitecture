...
- Use the simplest caching layer that solves the problem:
  ...
  - **Application cache** (Redis or Valkey, Memcached) for computed data, session data, or frequently queried database results.
...
- Never let a CDN or shared cache store a per-user response. Send `Cache-Control: private, no-store` on authenticated responses and `Vary` on any header that changes the output; cache-key mistakes here serve one user's data to another (see `rules/api.md`, `rules/security.md`).
- Give CDN-cached dynamic content an invalidation path from day one: purge by URL or by cache tag (`Cache-Tag` / `Surrogate-Key`) on the write that changes it, and use `stale-while-revalidate` so a refresh never turns into a miss storm. Content-hashed static assets need no purge. See `guides/performance/caching.md`.
