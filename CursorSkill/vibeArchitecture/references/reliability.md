...
- When two lock domains exist (a storage lock and a crypto-state lock, a cache lock and a session lock), define a single acquisition order and never nest them in the other direction — better yet, never nest them at all: gather what you need under one lock, release it, then take the other. Inconsistent nesting across two subsystems is a classic deadlock that freezes the whole process, passes every test, and only fires under production timing.
