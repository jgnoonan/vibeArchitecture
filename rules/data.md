...
- Know your runner's re-run behavior, and make migrations survive it. If migrations execute at application startup, a failing migration is not an error message — it's a production crash loop. If the runner re-executes files, DDL that references schema a *later* migration renamed or dropped must be guarded (conditional checks / DO-blocks). Cheap insurance either way: a "migrate twice" test that runs the full chain, then runs it again, and fails on any error.
